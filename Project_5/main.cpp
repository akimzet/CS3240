//	hash2.org	2013 may 28	last modified 2014 dec 02
//
...
{
	int count( ifstream &,int ),newerr( size_t );
	...
	if ( argc < 4 ) {
		cout << "** use is >hash2 file disp size1 [size2 ... ] **\n";
		exit( 1 );
	}
	f.open( *++argv,ios::in );
	if ( f.fail() ) {
...
find( const char *s,int size,int len )
{
	...
	char *p;
        ...
        {
            p = table[i].p;
            if((p != nill) &&(strcmp(s, p) == 0)) break;
            else
            {
                if(p == nill)
                {
                    if(ssp1 <= ssp + len) return(-1);
